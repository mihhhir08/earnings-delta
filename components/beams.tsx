...
function extendMaterial<T extends THREE.Material = THREE.Material>(
  ...
  Object.entries(config.uniforms ?? {}).forEach(([key, uniform]) => {
    uniforms[key] = uniform !== null && typeof uniform === "object" && "value" in uniform
...
function createStackedPlanesBufferGeometry(count: number, width: number, height: number, spacing: number, heightSegments: number) {
  const geometry = new THREE.BufferGeometry();
  const positions = new Float32Array(count * (heightSegments + 1) * 2 * 3);
  const indices = new Uint32Array(count * heightSegments * 2 * 3);
  const uvs = new Float32Array(count * (heightSegments + 1) * 2 * 2);
...
const DirectionalLight: FC<{ position: [number, number, number]; color: string }> = ({ position, color }) => {
  const light = useRef<THREE.DirectionalLight>(null!);
  useEffect(() => {
    const camera = light.current?.shadow.camera as (THREE.Camera & { top: number; bottom: number; left: number; right: number; far: number }) | undefined;
  ...
  return <directionalLight ref={light} color={color} intensity={1} position={position} />;
};
...
export default function Beams({
  ...
  const material = useMemo(() => extendMaterial(THREE.MeshStandardMaterial, {
    header: `
varying vec3 vEye;
...
gl_FragColor.rgb -= randomNoise / 15. * uNoiseIntensity;`,
  ...
  return (
    <CanvasWrapper reducedMotion={reducedMotion}>
      <group rotation={[0, 0, THREE.MathUtils.degToRad(rotation)]}>
        <MergedPlanes ref={meshRef} material={material} count={beamNumber} width={beamWidth} height={beamHeight} />
